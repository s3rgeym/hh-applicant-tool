...
В нижней части боковой панели находится переключатель аккаунтов. UI использует ту же систему профилей, что и CLI через `--profile` / `HH_PROFILE_ID`.

- `Основной` соответствует профилю `.`.
- `+ Добавить` создаёт отдельный профиль и сразу делает его активным.
- После создания профиля нужно войти в нужный аккаунт hh.ru через кнопку авторизации.
- Каждый профиль хранит собственные `config.json`, OAuth-токены, cookies, SQLite-базу, настройки и пресеты.
- Переключение профиля не требует перезапуска UI.
- Удаление профиля удаляет его локальные данные. Основной профиль удалить нельзя.
- Создание, переключение и удаление блокируются, пока выполняется авторизация или операция отклика.

Существующие CLI-сценарии с `hh-applicant-tool --profile <id> ...` и переменной `HH_PROFILE_ID` продолжают работать без изменений.

### Главная

Статус авторизации, активный профиль и имя пользователя. Показывает зелёный/красный индикатор подключения к hh.ru.
...
- **AI**: фильтр (heavy/light/custom), rate limit, системный промпт (для custom), промпт для письма
...
src/hh_applicant_tool/
  ui/
    ...
    profiles.py           # ProfilesManager — список/создание/переключение/удаление профилей
    templates/
...
```bash
pytest tests/test_ui_profiles.py tests/test_ui_api.py tests/test_ui_presets.py tests/test_ui_window.py tests/test_ui_multi_account_template.py -v
```
...
- В одном окне UI одновременно активен только один профиль. Параллельные отклики с нескольких аккаунтов не запускаются.
