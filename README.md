...
- 🧠 **AI-фильтрация вакансий** Можно автоматически фильтровать вакансии через AI, чтобы откликаться только на подходящие. AI анализирует полное резюме (должность, навыки, опыт работы) для более точной фильтрации вакансий.
...
Новая функция AI-фильтрации позволяет автоматически отсеивать неподходящие вакансии перед откликом. AI анализирует ваше резюме и сравнивает его с требованиями вакансии.

Пример использования AI-фильтрации:

```sh
hh-applicant-tool apply-vacancies --ai-filter
```

Дополнительные параметры для настройки AI-фильтрации:

- `--ai-rate-limit` - ограничение количества запросов к AI в минуту (по умолчанию 40)
- `--ai-prompt` - кастомный промпт для AI-фильтрации
