...
- 🔒 **Безопасность личных данных.** Ваши email, телефон, пароль и другие личные данные никуда не передаются в отличии от сторонних сервисов. В этом можно убедиться, изучив [открытый исходный код](https://github.com/s3rgeym/hh-applicant-tool/tree/main/applicant_tool). Владельцы сторонних сервисов никогда вам не покажут исходники. Они знают о вас все и эти данные спокойно продадут каким-нибудь жуликам, либо те утекут в результате взлома.
...
- [HH Applicant Tool](#hh-applicant-tool)
  ...
  - [Использование AI для генерации сопроводительного письма](#использование-ai-для-генерации-сопроводительного-письма)
    ...
    - [Эксперементальная поддержка BlackBox](#эксперементальная-поддержка-blackbox)
...
По этому пути находятся каталоги с профилями. В каталоге с профилем лежит главный конфиг `config.json`.

Полный путь до конфигурационного файла можно вывести с помощью команды:

```bash
hh-applicant-tool config -s
```

Для переноса конфига на новое устройство скопируйте директорию профиля либо файлы из нее, если надо создав директорию профиля по умолчанию `default`.
...
Данные о работодателях и их вакансиях хранятся в файле `data`. Это обычная база `sqlite3`.
...
- `-p <profile-id>` имя текущего профиля, подкаталога в каталоге с конфигурациями.

| Операция                  | Описание                                                                                                                                                                     |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **authorize**             | Открывает сайт hh.ru для авторизации                                                                                                                                         |
...
| **reply-chats**           | Ответить во все чаты с работодателями, где нет ответа либо не прочитали ваш предыдущий ответ                                                                                 |
...
| **get-employer-contacts** | Получить список полученных вами контактов работодателей. Поддерживается так же экспорт в html/jsonl.                                                                         |
...
По-сути, никакие дополнительные команды кроме имеющихся не нужны. Вы можете сделать что угодно с помощью `call-api`, но если хочется чего-то особенно, можно добавить свои команды.
