...
> Это рекомендованный способ разработчиком. Так же если не работает традиционная установка, то используйте его.

Это самый простой способ запуска и использования утилиты, требующий скопипастить 5 команд. Он подойдет обладателям выделенных серверов, используемых под VPN.
...
Обратите внимание, что `docker-compose exec`/`docker-compose run` запускаются с аргументами `-u docker`. Только для пользователя `docker` установлен `chromium`, необходимый для авторизации, а так же это избавляет от проблем с правами, когда созданный каталог с конфигами нельзя будет просмотреть без админских прав (sudo).
