...
- 🔒 **Безопасность личных данных.** Ваши email, телефон, пароль и другие личные данные никуда не передаются в отличии от сторонних сервисов. Если данная утилита и собирает телеметрию, то **СОБИРАЕТСЯ ТОЛЬКО ТЕЛЕМЕТРИЯ РАБОТОДАТЕЛЕЙ: ИХ КОНТАКТЫ, ИНФОРМАЦИЮ ОБ ОПУБЛИКОВАННЫХ ВАКАНСИЯХ И КОМПАНИИ — ВСЕ**. Это делается главным образом для отслеживания актуальных контактов работодателей и выявления мошенников среди них, но вы можете всегда ее удалить. В этом можно убедиться, изучив [открытый исходный код](https://github.com/s3rgeym/hh-applicant-tool/tree/main/hh_applicant_tool). Владельцы сторонних сервисов никогда вам не покажут исходники. Они знают о вас все и эти данные спокойно продадут каким-нибудь жуликам, либо те утекут в результате взлома.
...
#59218436 Соколов Дмитрий Игоревич (applicant) [ Резюме: 1 | Просмотры: +124 | Непрочитанных: +37 ]
```
...
Главный конфигурационный файл называется `config.json`. Он и логи в зависимости от операционной системы хранятся по следующим путям:
...
Для переноса конфига на новое устройство можете просто скопировать директорию или json-файл.
