...
- 🔒 **Ваши данные в безопасности.** Ваши email, телефон, пароль и другие личные данные никуда не передаются в отличии от сторонних сервисов. Если данная утилита и собирает телеметрию, то **СОБИРАЕТСЯ ТОЛЬКО ТЕЛЕМЕТРИЯ РАБОТОДАТЕЛЕЙ ИХ КОНТАКТЫ, ИНФОРМАЦИЮ ОБ ОПУБЛИКОВАННЫХ ВАКАНСИЯХ И КОМПАНИИ — ВСЕ**. Мало того, вы можете всегда ее удалить. В этом можно убедиться, изучив [открытый исходный код](https://github.com/s3rgeym/hh-applicant-tool/tree/main/hh_applicant_tool). Владельцы сторонних сервисов никогда вам не покажут исходники. Они знают о вас все и эти данные спокойно продадут каким-нибудь жуликам, либо те утекут в результате взлома.

- 🛡️ **Защита от блокировок.** Утилита выполняет запросы с вашего устройства, имитируя обычного пользователя. Сторонние сервисы, рассылающие запросы с одного сервера для сотен аккаунтов, что делает вероятность блокировки вашего аккаунта под 100%.
