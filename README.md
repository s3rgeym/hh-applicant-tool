...
Сайт хуй-хуй, созданный для овчарок, а поэтому имеющий красный логотип цвета менстуральных выделений, зарабатывает в тч на услугах по продвижению вакансий, когда вакансию показать некому, то он ее начинает подсовывать калдидатам, которых это "может" заинтересовать (хоть кому-то показать надо), так программистам предлагают вакансии учителей и наставников в онлайн-школах и прочую дичь. Ничего с этим не сделать...
...
Для разработки используется [Poetry](https://python-poetry.org/). Установка через `pipx`:

```bash
pipx install poetry
```

Установка зависимостей (включая dev-группу и все extras) и запуск тестов/линта:
